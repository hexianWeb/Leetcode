...
int merge(SSTABLE &L, int left, int mid, int right)
{
    ...
    {
        res += L.data[p1] > L.data[p2] ? (mid - p1 + 1) * 1 : 0;
...
// 合并排序
int process_reveser(SSTABLE &L, int left, int right)
{
    if (left == right)
    {
        return 0;
    ...
    return process_reveser(L, left, mid) +
           process_reveser(L, mid + 1, right) +
...
int main()
{
    ...
    InitSSTABLE(L);
    InsertNum(L);
    int num = process_reveser(L, 0, L.Length - 1);
    printf("逆序数总数为:%d\n", num);
